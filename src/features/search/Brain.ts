...
export const aiSubTurn = (
  ...
  for (const enemyUnit of enemyUnits) {
    ...
    for (const aiUnit of aiUnits) {
      ...
      const { minPath, bestTargetPos } = unitFirePathBfs(
        aiUnit,
        enemyUnit,
        units,
        grid
      ...
      if (
        minPath != null &&
        bestTargetPos != null &&
        minPath.length < pathLengthToUse
      ) {
        aiUnitToUse = aiUnit;
...
export const unitBfs = (
  ...
  return bfsHelper(
    head(mover),
    mover.stats.movement - mover.movesUsed,
    units,
    grid,
    target
  );
...
export const bfsHelper = (
  ...
  while (queue.length !== 0) {
    ...
    if (
      target != null &&
      manDistance(target, curNode.position) <= targetDistance
    ) {
