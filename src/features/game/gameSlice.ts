...
const unitHeadColor = (hue: number) => {
  return d3
    .cubehelix(hue, UNIT_SATURATION + 0.2, UNIT_LIGHTNESS - 0.3)
    .toString();
};
...
const locationValid = (
  ...
  return (
    Math.abs(newPosition.x - oldHead.x) + Math.abs(newPosition.y - oldHead.y) <=
      1 &&
...
const attackAction = (state: GameState, action: PayloadAction<Position>) => {
  ...
  if (
    attacker === target ||
    attacker.attackUsed ||
    !targetInRange(attacker, targetPosition)
  ) {
    return;
  }
...
export const gameSlice = createSlice({
  ...
  reducers: {
    ...
    clickMove: (state: GameState) => {
      ...
      if (
        selectedUnit &&
        selectedUnit.movesUsed < selectedUnit.stats.movement
      ) {
        state.phase = "move";
      }
    },
...
const getActivePlayerUnits = (state: GameState) =>
...
const getActivePlayer = (state: GameState) =>
  state.players[state.turn % state.players.length];
...
const deleteDeadPlayers = (state: GameState) => {
  state.players = state.players.filter(
    (player) => getUnitsForPlayer(state, player).length > 0
  );
};

// Regular Selectors
export const selectUnitList = (state: RootState) => getUnitList(state.game);
export const selectActivePlayerUnits = (state: RootState) =>
  getActivePlayerUnits(state.game);
export const selectActivePlayer = (state: RootState) =>
  getActivePlayer(state.game);
export const selectSelectedUnit = (state: RootState) =>
  getSelectedUnit(state.game);
